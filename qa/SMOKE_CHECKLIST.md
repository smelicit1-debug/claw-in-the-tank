# Claw in the Tank — Smoke Test Checklist
...
**Setup:**
1. Build the candidate image: `docker build -f packages/integration/Dockerfile -t citt:smoke .`
2. Reset state: `docker rm -f citt-smoke 2>/dev/null; docker volume rm citt-smoke-data 2>/dev/null; docker volume create citt-smoke-data`
3. Start: `docker run -d --name citt-smoke --cap-add=NET_ADMIN --device /dev/net/tun --sysctl net.ipv4.ip_forward=1 -p 127.0.0.1:8788:8787 -v citt-smoke-data:/data citt:smoke`
...
- [ ] **A3.** `docker exec citt-smoke supervisorctl -c /etc/supervisor/supervisord.conf status` — all programs RUNNING except `llama-server` (which should be STOPPED unless local fallback is enabled)
- [ ] **A4.** Architecture check on Apple Silicon: `docker exec citt-smoke ls /app/llama-cpp/ | grep libggml-cpu | head -3` — output should contain `armv8` or `armv9` flavors (NOT `alderlake`/`cannonlake`/etc which would be x86_64 — that's #114)
...
- [ ] **C2.** Verify `docker exec citt-smoke ls /data/state/webui/settings.json` exists, foxinthebox-owned (NOT in `/app/.hermes/webui/`)
- [ ] **C3.** `docker restart citt-smoke`, wait for /health
...
- [ ] **D5.** Verify config landed in the gateway's path: `docker exec citt-smoke cat /data/data/hermes/config.yaml | head -10` shows `provider: custom` and the Ollama base URL (NOT in `/app/.hermes/config.yaml`)
...
- [ ] **E9.** No orphan `tailscale up` processes: `docker exec citt-smoke ps aux 2>/dev/null | grep "tailscale up" | grep -v grep` returns nothing
...
- [ ] **F7.** Direct chat works: `docker exec citt-smoke curl -s -X POST http://127.0.0.1:8643/v1/chat/completions -H 'Content-Type: application/json' -d '{"model":"phi4-mini","messages":[{"role":"user","content":"Reply PONG"}],"max_tokens":5,"stream":false}'` returns a real completion
...
```bash
docker run -d --name claw-in-the-tank \
  --cap-add=NET_ADMIN --device /dev/net/tun --sysctl net.ipv4.ip_forward=1 \
  -p 127.0.0.1:8787:8787 \
  -v "$HOME/Library/Application Support/Claw in the Tank:/data" \
