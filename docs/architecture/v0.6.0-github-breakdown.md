...
| `fork-side` | PRs that land in claw-in-the-box-ai/hermes-{agent,webui} | orange |
| `monorepo` | PRs that land in claw-in-the-box-ai/claw-in-the-box | orange (light) |
...
```markdown
This epic tracks the full migration of Claw in the Box's dependency on
...
- [ ] No open PR on claw-in-the-box-ai/hermes-{webui,agent} forks.
...
- [ ] Branch protection on claw-in-the-box-ai/hermes-{webui,agent} does NOT block
      the `citb-overlay-archive-YYYY-MM-DD` tag push (Phase 10).
...
```markdown
Send 5 bug-fix-class CITB commits as upstream PRs. Each accepted PR shrinks
...
- **Branch:** `citb/phase2-move-static`
...
- **Branch:** `citb/phase3-restore-upstream`
- **Prereq:** Phase 1 (#11) merged
...
- **Repo:** fork
- **Branch:** `citb/phase4-restore-upstream-server-routes`
...
- Branch: `citb/phase5-remove-<module>`
...
- **Repo:** fork
- **Branch:** `citb/phase6-restore-mid-file-edits`
...
- **Repo:** fork
- **Branch:** `citb/phase7-remove-onboarding`
...
- **Prereq:** PR #51 merged
- **Deliverable:**
  - Tag current `master`/`main` heads on `claw-in-the-box-ai/hermes-{webui,agent}` as `citb-overlay-archive-YYYY-MM-DD`
  - Push tags
