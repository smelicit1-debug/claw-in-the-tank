...
The single highest-value early action is **Phase 0 (Upstream-PR campaign)**: send the 5 bug-fix-class CITT commits to NousResearch and nesquena. Each accepted PR shrinks the patch series forever, and three of the five (`runtime_provider.py`, `auxiliary_client.py`, `models.py` #1558) are textbook upstream candidates.
...
- [ ] No open PR on the claw-in-the-tank-ai/hermes-{webui,agent} forks awaiting merge — they will be invalidated by the submodule re-point.
...
- [ ] Branch protection rules on `claw-in-the-tank-ai/hermes-{webui,agent}` are confirmed not to block the planned `citt-overlay-archive` tag push (Phase 7 step 7.3).
...
|---|---|---|---|---|
| 1 | `hermes_cli/runtime_provider.py` | NousResearch/hermes-agent | `citt/fix-target-model-bedrock` | One-line `target_model` fix for Bedrock api_mode routing |
| 2 | `api/models.py` (#1558) | nesquena/hermes-webui | `citt/fix-1558-metadata-save` | P0 metadata-only Session.save guard (data-loss class) |
| 3 | `agent/auxiliary_client.py` | NousResearch/hermes-agent | `citt/fix-provider-auto-fallback` | `provider=auto` resolution + `auxiliary.default` fallback |
| 4 | `cron/{jobs,scheduler}.py` + `tools/cronjob_tools.py` | NousResearch/hermes-agent | `citt/feat-cron-failure-diagnostics` | Rolling-5 failure history + diagnostics fields |
| 5 | `api/providers.py` | nesquena/hermes-webui | `citt/fix-providers-hot-reload` | Gateway hot-reload after key change |

**Validation gates.** Each upstream PR must (a) include unit tests if the file has tests, (b) declare `Upstream-Status: Submitted <PR-URL>` in the corresponding Fox commit message in the standalone clone, (c) reference the CITT issue if any.
...
name = "fox-overlay"
version = "0.0.1"
description = "Claw in the Tank overlay for hermes-agent and hermes-webui"
...
```
forks/hermes-webui/static/claw-in-the-tank.css   → packages/fox-overlay/webui_static/claw-in-the-tank.css
forks/hermes-webui/static/claw-in-the-tank.js    → packages/fox-overlay/webui_static/fox-overlay.js
...
Edit `forks/hermes-webui/static/index.html` (in the standalone fork): remove the explicit `<link>` to `claw-in-the-tank.css` and the explicit `<script>` for `claw-in-the-tank.js` — the `extensions.py` injector will now provide them.
...
- View-source comparison: `curl http://127.0.0.1:8788/` before vs after must contain (a) the `<link rel="stylesheet" href="/extensions/claw-in-the-tank.css">` injection, (b) the `<script defer src="/extensions/fox-overlay.js">` injection, (c) all other markup byte-identical except those two diffs.
- HTTP status 200 on `/extensions/claw-in-the-tank.css`, `/extensions/fox-overlay.js`, `/extensions/setup.html`, `/extensions/setup.js`, `/extensions/setup.css`, all font files.
...
**Risks.**
- Some upstream code (e.g. `static/index.html` or `boot.js`) hard-references one of the moved file paths (e.g. `/static/claw-in-the-tank.css` literal). **Mitigation:** `grep -r "claw-in-the-tank\|fox_avatar\|setup.html\|setup.js" forks/hermes-webui/` before moving; rewrite or proxy.
...
In hermes-agent fork (standalone clone), branch `citt/phase3-restore-upstream`:
...
In hermes-webui standalone fork, on a `citt/phase4-restore-upstream-server-routes` branch: revert the existing Fox edits to `server.py` and `api/routes.py`. The patches in monorepo are now the source of truth for those edits.
...
- Fox's `local-patches` branch in `smelicit1-debug/hermes-webui` (or repurpose as `citt-overlay-archive` tag for historical reference).
...
- `docker exec citt-test cat /app/version.txt` returns the FITB_VERSION (proves no `version.txt` regression).
- `docker exec citt-test ls /app/hermes-webui/api/` shows neither `ollama.py` nor `tailscale.py` etc. — they're in `/app/fox-overlay/...`.
- `docker exec citt-test pip show hermes-webui` shows the pinned upstream version (e.g. `Version: 0.51.22`).
...
Critically: the prior Fox-fork branches (`master`/`main`) on `claw-in-the-tank-ai/hermes-{webui,agent}` are **not deleted** until Phase 9. Until then, rollback is fully reversible.
...
In fork repos (`claw-in-the-tank-ai/hermes-{webui,agent}`):
- Tag current `master`/`main` as `citt-overlay-archive-2026-MM-DD` so the historical Fox-fork content remains discoverable.
...
- **Verification container** — The convention from `reference_test_container.md`: `citt-test` on port 8788 with `--cap-add=NET_ADMIN --device /dev/net/tun --sysctl net.ipv4.ip_forward=1` and a named volume. Required for Tailscale section of smoke to actually work.
