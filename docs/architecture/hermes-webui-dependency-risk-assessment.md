...
- `HERMES_WEBUI_EXTENSION_*`: script/style injection. Fox does NOT yet use; could replace some of `static/claw-in-the-tank.{css,js}` overlay.
...
- *Reaction time:* Fox's overlay (`static/claw-in-the-tank.{css,js}`, all of `static/panels.js` modifications) is built against vanilla-JS DOM. A React rewrite would invalidate **100% of Fox's static/ patches** (the ~3,000+ LOC of static modifications).
