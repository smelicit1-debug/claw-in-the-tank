...
Messages queued in Claw in the Tank WebUI are properly queued and drained by the system, but **user responses are not visible** after queue processing completes. The queue mechanism works (messages get dequeued), but the UI fails to display the assistant's responses, making it appear like messages are "lost in queue."
