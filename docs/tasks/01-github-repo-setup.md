...
- [ ] GitHub organization `claw-in-the-box-ai` exists and is accessible
- [ ] Repository `claw-in-the-box-ai/claw-in-the-box` exists as a fresh (empty or minimal) repo with `main` as the default branch
...
- [ ] All three repositories are visible under the `claw-in-the-box-ai` org
...
4. **Fork hermes-webui**
   ...
   - *(For reference when unblocked:)* Click **Fork** on the upstream repo, set
     Owner: `claw-in-the-box-ai`, Repository name: `hermes-webui`, default branch `main`
...
git clone https://github.com/smelicit1-debug/claw-in-the-box.git
...
git -C claw-in-the-box branch -r
...
- The `claw-in-the-box` monorepo is a **fresh repo**, not a fork of anything.
