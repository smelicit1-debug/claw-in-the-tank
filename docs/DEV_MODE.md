...
- Working on a feature branch in either submodule and want to run it inside the CITT container immediately
...
- Tags the image as `claw-in-the-tank:dev`
...
| Aspect | `claw-in-the-tank:dev` | `claw-in-the-tank:<version>` (production) |
|---|---|---|
...
docker build -f packages/integration/Dockerfile \
  -t citt:repro --build-arg FITB_VERSION=v0.5.0 .
docker run --rm --cap-add=NET_ADMIN --device /dev/net/tun \
  --sysctl net.ipv4.ip_forward=1 \
  -p 127.0.0.1:8788:8787 \
  -v citt-repro-data:/data citt:repro
```
